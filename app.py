...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 1rem 0 0.5rem;">
      ...
      <div style="font-weight:900; font-size:1rem; color:#ecf0f1; letter-spacing:-0.5px;">치안 과학기술 동향</div>
      <div style="font-size:0.7rem; color:#7f8c8d; margin-top:2px;">KIPOT Intelligence Platform</div>
    </div>
    """, unsafe_allow_html=True)
    st.divider()
    ...
    st.markdown(f"""
    <div style="font-size:0.7rem; color:#5d6d7e; text-align:center; padding:8px 0;">
      🕐 마지막 업데이트<br>
      <b style="color:#7f8c8d;">{avail_dates[0] if avail_dates else "-"} 09:00 KST</b>
    </div>
    """, unsafe_allow_html=True)
        ...
        for col, (domain, color) in zip(grid_cols, DOMAINS):
            ...
            with col:
                ...
                st.markdown(
                    ...
                    f"border-top:3px solid {color}; border-radius:8px 8px 0 0; "
                    f"padding:0.6rem 0.8rem 0.4rem; font-size:0.87rem; font-weight:700; "
                    f"color:{color}; line-height:1.3;'>{domain}{new_label}</div>",
                    unsafe_allow_html=True
                )
                # 숫자 + 버튼을 같은 컬럼 3분할로
                nc1, nc2, nc3 = st.columns(3)
                with nc1:
                    st.metric("이슈", issue_cnt)
                    if st.button("보기", key=f"btn_iss_{domain}",
                                 use_container_width=True, disabled=(issue_cnt == 0)):
                        st.session_state.dash_domain = domain
                        st.session_state.dash_filter = "issues"
                        st.rerun()
                with nc2:
                    st.metric("기술", tech_cnt)
                    if st.button("보기", key=f"btn_tec_{domain}",
                                 use_container_width=True, disabled=(tech_cnt == 0)):
                        st.session_state.dash_domain = domain
                        st.session_state.dash_filter = "techs"
                        st.rerun()
                with nc3:
                    st.metric("합계", total_cnt)
                    if st.button("전체", key=f"btn_all_{domain}",
...
**🔗 [관련 기관/연구소 바로가기]({tech.get('url','#')})**
                ...
                with st.expander(f"📂 기술 심층 데이터 — {tech.get('title','')[:35]}…"):
                    c1, c2, c3 = st.columns(3)
                    c1.metric("기술 성숙도(TRL)", f"{trl} / 9")
                    c2.metric("분류 도메인", tech.get('domain','').split(' ')[0])
                    c3.metric("분류 태그 수", len(tech.get('tags',[])))
                    st.markdown(f"""
**기술 상세 설명:**
{tech.get('detail','')}
